...
const Profile = () => {
  ...
  const onSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) return;
    const parsed = schema.safeParse({ full_name, dob, gender });
    if (!parsed.success) { toast.error(parsed.error.issues[0].message); return; }
    setSaving(true);
    const payload: any = { user_id: user.id, full_name, dob };
      ...
      <div className="max-w-2xl mx-auto animate-fade-up">
        ...
        <Card className="p-8 gradient-card shadow-soft border-border/50">
          <form onSubmit={onSave} className="space-y-6">
            ...
            {/* เพศ ถูกซ่อนจากหน้าโปรไฟล์ ตามคำสั่ง: ไม่แสดงฟิลด์เพศที่นี่ แต่เก็บค่าไว้ในฐานข้อมูล */}
            <div className="space-y-2">
